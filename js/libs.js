$(document).ready(function(){
	...
	$('body').on('click','.ft-soc .fa',function(){
		var $url = $(this).attr("data-href"); window.open($url);
	});
	
	if ($(window).width() > 1220) {
		$('.hmenu li:has(.hidden-menu)').hover(function() {
			$(this).children('.hidden-menu').stop(true,true).fadeToggle(300);
			$(this).toggleClass('menuactive');
		});	
		$('.logged li:has(.login-box2)').hover(function() {
			$(this).children('.login-box2').stop(true,true).fadeToggle(300);
			$(this).toggleClass('is-active');
		});	
	} else {
		$('.logged li:has(.login-box2) > a').click(function() {
			$(this).next('.login-box2').stop(true,true).fadeToggle(300);
	...
	$('.search-box input').focus(function(){
		if ( $('.focused').length < 1 ) { $('body').append('<div class="focused"></div>'); };
		$('.focused').animate({bottom: '0'}, 200);
	});
	
	$('.search-box input').blur(function(){
		$('.focused').animate({bottom: '-100%'}, 200);
	...
	function callback(event) { setTimeout(function() { $(window).lazyLoadXT(); }, 300); };
	 ...
	 $("#owl-slide").owlCarousel({
		items:1, loop:false, rewind:true, nav:true,
		navText:['<span class="fa fa-angle-left"></span>','<span class="fa fa-angle-right"></span>'],
		slideBy:1, dots:true, autoplay:false, autoplayTimeout:12000, animateOut: 'fadeOut', 
		onInitialized: callback, onTranslate: callback
	 });
	
	 $(".sect-slide-items .owl-carousel").owlCarousel({
		items:5, loop:false, rewind:true, nav:true,
		navText:['<span class="fa fa-angle-left"></span>','<span class="fa fa-angle-right"></span>'],
		slideBy:2, dots:false, autoplay:false, autoplayTimeout:12000, margin:30,
		autoWidth:true, autoplayHoverPause:true, onInitialized: callback, onTranslate: callback, 
		responsive:{0 : {margin:15, nav:true, autoplayTimeout:6000, slideBy:1}, 470 : {margin:15, nav:true, autoplayTimeout:12000, slideBy:2}, 1220 :{margin:30} }
	 });
	 
	$('.th-rate').each(function(){
        var a = $(this),
			b = parseInt(a.find('.th-rate-in .ratingtypeplusminus').text()),
			c = parseInt(a.find('.th-rate-in span[id]:last').text()),
			d = parseInt(a.children('span').text());
        if ( c >= b ) {
        var m = (c - b)/2,
			p = c - m;
		a.find('.sr-plus').append('<span class="psc">'+p+'</span>');
        a.find('.sr-minus').append('<span class="msc">'+m+'</span>');
        };
		if ( d > 0 ) { a.addClass('high'); } 
		else if ( d < 0 ) { a.addClass('low'); };
		a.addClass('rdone');
    });
	...
	if ($(window).width() > 1220) {	
		...
		function removeThtext() {
			...
			$('body').children('#trailer-box').remove();
			}, 100);
		};
		...
		$('body').on({
			mouseenter: function () {
				...
				var inf = $(this), iUrl = inf.closest('.th-img').attr('href'),
					...
					if( $('.th-text').length < 1 ) {
						$.ajax({
							...
							success: function(data) {
								...
								iText.empty();
								iText.append($('.minfo-wr-in', data)).append($('.mtext', data)).append($('.mh-right', data));
								iText.append('<div class="tbtns"><div class="tbtn ttrl button" title="Трейлер">Смотреть трейлер</div></div>');
								$('body').children('#trailer-box').remove();
								$('body').append('<div id="trailer-box" class="video-box" title="Трейлер"></div>');
							   ...
							   $("#trailer-box").append('<script type="text/javascript" src="/engine/classes/html5player/player.js"></script><link media="screen" href="/engine/classes/html5player/player.css" type="text/css" rel="stylesheet" />');
								...
								$('.rate3-short:not(.rdone)').each(function(){
									...
									var m = (c - b)/2,
										...
										id = a.data('id'),
										srate = $("#srate3-" + id);
									a.children('.sr-plus1').removeAttr('onclick').append('<span class="psc">'+p+'</span>');
									a.children('.sr-minus1').removeAttr('onclick').append('<span class="msc">'+m+'</span>');
        ...
        if ($urlNext !== undefined) {
			$.ajax({
                ...
                success: function(data) {
					...
					$('.th-rate:not(.rdone)').each(function(){
						var a = $(this),
							b = parseInt(a.find('.th-rate-in .ratingtypeplusminus').text()),
							c = parseInt(a.find('.th-rate-in span[id]:last').text()),
							d = parseInt(a.children('span').text());
   ...
   $('.slice-this').wTextSlicer({
	...
	$('.top2-items').each(function(){
		var first = $(this).find('.top2-item:first'), firstImg = first.find('img'), firstLink = first.attr('href'),
		target = $(this).siblings('.top2-big-poster').children('.top2-big-poster-in');
		first.addClass('current');
		firstImg.clone().prependTo(target);
		target.attr('href', firstLink).addClass('is-active');
	});
	$('body').on({
			mouseenter: function () {
				var first = $(this), firstImg = first.find('img'), firstLink = first.attr('href'),
				target = $(this).parent().siblings('.top2-big-poster');
				first.siblings().removeClass('current');
				target.empty();
				first.addClass('current');
				firstImg.clone().prependTo(target);
				target.children().wrap('<a class="top2-big-poster-in img-box" href="'+firstLink+'"></a>');
				target.children().animate({ opacity: '1', top: '0' }, 100); 
				setTimeout(function() { $(window).lazyLoadXT(); }, 300);
				},
			mouseleave: function () {}
		},'.top2-item:not(.current)');
	...
	$('body').append('<div class="close-overlay" id="close-overlay"></div><ul class="side-panel" id="side-panel"></ul><div class="btn-close"><span class="fa fa-times"></span></div>');
	$('.hmenu').each(function() {
